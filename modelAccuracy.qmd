...
Suppose that we flip 1,024 fair coins ten times each. Then we would expect (on average) one coin to come up *all tails*. (There’s a $\frac{1}{2}^{10} = \frac{1}{1,024}$ chance that any single coin will come up all tails. So if we flip 1,024 coins, then we expect one coin to come up all tails, on average). If one of our coins comes up all tails, then
we might therefore conclude that this particular coin is not fair. In fact, a standard hypothesis test for the null hypothesis that this particular coin is fair would lead to a p-value below $0.002^{12}$ But it would be incorrect to
conclude that the coin is not fair: in fact, the null hypothesis holds, and we just happen to have gotten ten tails in a row by chance.
...
How can we control fot this situation?
...
# Define the number of hypotheses
num_hypotheses <- seq(1, 500, by = 1)

# Define the alpha values
alpha_values <- c(0.05, 0.01, 0.001)

# Calculate the FWER for each alpha value and number of hypotheses
fwer <- function(alpha, m) {
  1 - (1 - alpha)^m
}

# Create a data frame for plotting
fwer_df <- expand.grid(Hypotheses = num_hypotheses, Alpha = alpha_values) %>%
  mutate(FWER = fwer(Alpha, Hypotheses))

# Plot the FWER for different alpha values and numbers of hypotheses
ggplot(fwer_df, aes(x = Hypotheses, y = FWER, color = factor(Alpha))) +
  geom_line(size = 1) +
  labs(title = "Family-Wise Error Rate vs. Number of Hypotheses (Log Scale)",
       x = "Number of Hypotheses (log scale)",
       y = "Family-Wise Error Rate",
       color = "Alpha") +
  theme_minimal() +
  scale_color_manual(values = c("0.05" = "yellow", "0.01" = "blue", "0.001" = "black")) +
...
The *Bonferroni* method adjusts the significance level (𝛼) for each individual hypothesis test to ensure that the overall probability of making at least one Type I error (false positive) remains controlled.
Given a number of hypotheses being tested $m$, we adjust the significance level for each test $\alpha_{adj}$ by dividing the desired significance level $\alpha$ byt the number of hypotheses $m$ 
$$
...
We now consider the Fund dataset. It contains data for 50 months for a list of managers and their excess return, this is, the percentage points difference between the funds managed by these people compared with the stock market in general for the same period.
In the table below we show the stats for the first 5 manangers in our list. 
...
- Compute the $p$-values for each of our hypotheses
...
We are going to use now the full dataset of Fund with 2000 managers and apply the Bonferroni method and the Benjamini-Hochberg method.
...
for (i in df$pValues){
  df$BHreject = df$pValues < (alpha*j)/m
  j=j+1
}

ggplot(df, aes(x = manager, y = pValues, color = BHreject)) +
  geom_point() +
  scale_x_log10()+
  scale_y_continuous(limits = c(0, 1)) +
  labs(title = "Plot of log(manager) vs pvalues",
       x = "log(manager)",
       y = "pvalues",
       color = "BHreject") +
  theme_minimal()
...
sum(df$BHreject)
```
Due to the large number of $m$, the Bonferroni method does not reject any hypothesys, while Benjamin-Hochberg rejects 289, but we have to remember that we expect 5% of them will be false rejections at our level of alpha. 
...
We have two samples $X$ and $Y$ and the $H_0$ is that the means of those two samples is the same. As we have seen in the central limit theorem if N is big enough, we know that T approximately follows a normal distribution under $H_0$, but if n is small, then we don't know the theoretical null distribution of T, so we can can use re sampling and let the computer find out the right distribution. 
 ...
 - the $p$-value is given by the fraction of the times that the $t$-statistic on the shuffled data exceeded an absolute value, the $t$-statistic on the real data.
