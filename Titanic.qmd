...
```{r, fig.align='center', fig.width=6} library(bestNormalize)}
...
To check how it is doing over the test data, we need to also drop the variables that we are not using in the model for this dataset and apply the same transformation that we did to the train dataset:
Treat NAs

```{r}
summary(titanic_test)
```
There are 22 rows without survival data. We will drop those rows as they will not help assessing the model.
...
Now we apply the model to the dataset:
...
# Confusion matrix (assuming titanic_test_encoded$Survived is available for evaluation)
table(Predicted = pred_classes, Actual = titanic_test_normalized$Survived)
