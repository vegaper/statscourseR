...
```{r, fig.align='center', fig.width=8}
correlation_matrix_dummies <- cor(numerical_correlation_data_scaled)  # Create the correlation plot
corrplot::corrplot(correlation_matrix_dummies,
  method = "circle", 
  type = "upper",   
  tl.col = "black",     
...
knitr::kable(anova_df, caption = "One-Way ANOVA P-values by Feature")
...
```{r}
numerical_correlation_data_scaled <- numerical_correlation_data_scaled %>% 
...
model <- glm(Survived ~ ., data = numerical_correlation_data_scaled, family = binomial)
...
```{r}
train_conf <- table(Predicted = train_pred_classes, Actual = numerical_correlation_data_scaled$Survived)
train_conf
```

or using `caret` 
```{r}
conf_train <- confusionMatrix(as.factor(train_pred_classes), as.factor(numerical_correlation_data_scaled$Survived))
conf_train
```
...
train_features <- colnames(numerical_correlation_data_scaled)
...
```{r, fig.align='center', fig.width=6}
# Predict on the test set (assuming titanic_test_encoded is prepared similarly)
...
we can also use the `caret` package to create a confusion matrix and get a summary of the results:
...
cm<- caret::confusionMatrix(pred_classes_factor, true_classes_factor)
cm
```
...
# Generate ROC curve
roc_obj <- roc(titanic_test_normalized$Survived, pred_probs)  # 'probs' is your predicted probabilities
...
This is especially useful if the positive class is rare (like survivors in Titanic).
...
*Precision*: Out of all passengers the model said survived, what proportion actually did.

*Recall (Sensitivity)*: Out of all passengers who actually survived, how many did the model correctly identify.

So on the graph:

The x-axis is Recall: how many actual survivors we captured.

The y-axis is Precision: how cleanly we predicted survivors without mixing in false positives.
...
train_df <- data.frame(
  Actual = numerical_correlation_data_scaled$Survived,
...
train_mat <- table(Predicted = train_pred_classes, Actual = numerical_correlation_data_scaled$Survived)
