...
Variance has a linear property: Let $X$ and $Y$ be random variables with $\text{Var}(X) = \sigma_X^2$ and $\text{Var}(Y) = \sigma_Y^2$. It is also necessary to assume that $X$ and $Y$ are independent. Suppose we are interested in a new random variable $Z = aX + bY + c$ where $a$, $b$, and $c$ are any real constants. The variance of $Z$ is then $\text{Var(Z)} = \text{Var(aX + bY + c)} =a^2 Var(X)+b^2 Var(Y)+0 = a^2\sigma_X^2+b^2\sigma_Y^2$. Because $c$ is constant, it has variance $0$.
...
The expected value, also known as the expectation or mean, of a random variable $X$ is denoted $E(X)$. It is the weighed average of all values $X$ could take, with weights given by the probabilities of those values. If $X$ is discrete-valued, then
...
::: callout-grey
We can also compute expectations of functions of $X$. For example, suppose $g(X) = 2/X$. Then we have: $$
E(g(X)) = \int_{0}^{1} \frac{2}{x} f(x) dx
$$

Note however that in general, $E(g(X)) \neq g(E(X))$.

Example: Let's say a continuous random variable $X$ has PDF $f(x) = 3x^2 I_{\{0 \leq x \leq 1\}}(x)$. We want to find $E(X)$ and $E(X^2)$. First,

$$
E(X) = \int_{-\infty}^{\infty} x \cdot 3x^2 I_{\{0 \leq x \leq 1\}}(x) dx
$$ $$
= \int_{0}^{1} x \cdot 3x^2 dx 
$$ $$
= \int_{0}^{1} 3x^3 dx = \left[ \frac{3}{4} x^4 \right]_0^1 = \frac{3}{4}(1-0)
$$

And second, $$
E(X^2) = \int_{-\infty}^{\infty} x^2 \cdot 3x^2 I_{\{0 \leq x \leq 1\}}(x) dx 
$$ $$
 = \int_{0}^{1} 3x^4 dx = \left[ \frac{3}{5} x^5 \right]_0^1 = \frac{3}{5}(1-0)
$$
:::
...
A CDF gives the probability that a random variable will take a value less than or equal to a specific value. It accumulates the probabilities from the [PDF](#pdf)
...
When the CDF is derived from data, as opposed to theoretically, we also call it the *empirical CDF* (ECDF).
...
The cumulative distribution function exists for every distribution. We define it as $F(x) = P(x \leq x)$ for a random variable X. If $X$ is discrete-valued, then the CDF is computed with summation $F(x) = \sum _{t=-\infty}^xf(t)$ where $f(t) = P(X=t)$ is the probability mass function (PMF).

Example:

Suppose $X \sim \text{Binomial}(5, 0.6)$. Then $$F(1) = P(X \leq 1) = \sum_{t=0}^{1} \binom{5}{t} 0.6^t (1 - 0.6)^{5-t} =\\  \binom{5}{0}0.6^0(1 - 0.6)^5 + \binom{5}{1}0.6^1(1 - 0.6)^4 \approx 0.087$$
...
If $X$ is continuous, the CDF is computed with an integral $f(x) = \int^x_{-\infty}f(t)$ where $f(t)$ is the Probability Density Function (PDF) Example exponential distribution: Suppose $Y \sim \text{Exp}(1)$. Then $$F(2) = P(Y \leq 2) = \int_{0}^{2} e^{-t} \,dt = 1 - e^{-2} \approx 0.865$$ The CDF is convenient for calculating probabilities of intervals. Let $a$ and $b$ be any real numbers with $a < b$. Then the probability that $X$ falls between $a$ and $b$ is equal to $P(a < X \leq b) = P(X \leq b) - P(X \leq a) = F(b) - F(a)$. This concept is illustrated in Figure 1.
...
abline(h = c(pnorm(-1), pnorm(1)), col = "red")
text(-2, 0.5, "F(-1)", pos = 2)
text(2, 0.5, "F(1)", pos = 4)

```

# The Probability Density Function (PDF) {#pdf}

A PDF describes the likelihood of a random variable taking on a specific value. It is used for continuous variables. The area under a curve of a PDF over a certain interval represents the probability of the random variable falling within that interval. The PDF itself is not a probability, but a density.
...
x <- seq(-3, 3, length = 100)
y <- dnorm(x)
plot(x, y, type = "l", main = "P(-1)")

polygon(c(-3, seq(-3, -1, length=100), -1), 
        c(-3, dnorm(seq(-3, -1, length=100)), 0), 
...
plot(x, y, type = "l", main = "P(-1 <= 1)")
polygon(c(-1, seq(-1, 1, length=100), 1),
        c(0, dnorm(seq(-1, 1, length=100)), 0),
        col=rgb(0.1, 0.1, 0.9, 0.2), border=NA)
...
Example: In a standardized test, the 97th percentile of scores among all test-takers is 23. Then 23 is the score you must achieve on the test in order to score higher than 97% of all test-takers. We could equivalently call $q = 23$ the 0.97 quantile of the distribution of test scores.

Later in [this document](/exploratoryAnalysis.html#qq-plot) we will see how we will use this to check for normal distributions in a sample.
...
# Probability Distributions functions in R

Each of the distributions introduced have convenient functions in R which allow you to evaluate the PDF/PMF, CDF, and quantile functions, as well as generate random samples from the distribution.

dnorm(x, mean, sd) Evaluate the PDF at x (mean = μ and sd = σ). pnorm(q, mean, sd) Evaluate the CDF at q. qnorm(p, mean, sd) Evaluate the quantile function at p. rnorm(n, mean, sd) Generate n pseudo-random samples from the normal distribution.

These four functions exist for each distribution, where the d... function evaluates the density/mass, p... evaluates the CDF, q... evaluates the quantile, and r... generates a sample.
...
| N(μ, σ²)       | `dnorm(x, mean, sd)`       | mean = μ, sd = √σ²     |
| t_ν            | `dt(x, df)`                | df = ν                 |
...
# Use of standardization in binomial probability
