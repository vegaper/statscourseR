...
-   Statistical Learning (Stanford Online STATSX0001 via Edx)
...
$$
\sum_{i=1}^{n} (y_i - \hat{y}_i)^2 = \sum_{i=1}^{n} \left( y_i - (\hat{\beta_o} + \hat{\beta_1} x_i) \right)^2
$$ 
$$
...
The observed values of Y will not fall directly over the regression line. At each value of X=x there is typically a distribution of possible Y values.
...
### Measuring the Quality of Fit. Mean Squared Error
...
In this simle example above we only have two k elements (0 and 1) but there may be many K elements, so we express the probability of k for a given value of x like this: $$
