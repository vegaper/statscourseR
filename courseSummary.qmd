...
```{r boxplot_showing_outlier, fig.cap="Normally distributed data with one point that is very large due to a mistake.", fig.align='center'}
set.seed(1)
...
```{r, scatter_plot_showing_outlier,fig.cap="Scatterplot showing bivariate normal data with one signal outlier resulting in large values in both dimensions.", fig.align='center' }
set.seed(1)
x[23] <- 100 ##mistake made in 23th measurement
...
```{r spearman_corr_illustration, fig.cap="Scatterplot of original data (left) and ranks (right). Spearman correlation reduces the influence of outliers by considering the ranks instead of original data.",fig.width=10.5,fig.height=5.25, fig.align='center'}
mypar(1,2)
...
```{r ecdffunction, fig.align='center'}
...
```{r histogramofnormaldist,fig.align='center'}
...
```{r null_and_obs,fig.cap="Null distribution with observed difference marked with vertical red line.", fig.align='center'}
hist(null, freq=TRUE)
...
```{r empiricalRule, fig.height=4,fig.width=6, fig.align='center'}
...
To see this with an example let's calculate the number of individuals that are taller than 72 inches. First we are going to calculate the number manually:
...
so we have roughly 6.4% of the individuals taller than 72 inches. Using normal approximation we can calculate it like this and it will gives us an similar number:

```{r}
1-pnorm(72,mean(heights),popsd(heights)) 
```
...
normalqs <- qnorm(ps,mean(heights),popsd(heights))
...
and to calculate the probability we use the coefficient with the probability formula:
...
If we have more than two outcomes but we are interested in one of them only we can still use the binomial formula. For example, we are playing an online game, the probability of winning a big prize is 10%, the probability of winning a small prize is 20% and the probability of not winning anything is 70%. We want to know, in 10 repetitions, what is the probability that we win two small prizes:
...
To standardize data for a binomial experiment, you typically use the **z-score** formula. This formula converts the binomial distribution to a standard normal distribution. The z-score formula is: $$
z=\frac{X-\mu}{\sigma}
$$
...
-   ( $\sigma$ ) is the standard deviation of the binomial distribution, calculated as $\sigma = \sqrt{np(1-p)}$ .
...
Where q = z-Score value. lower.tail: If TRUE, the probability in the normal distribution to the left of zScore is returned. The probability to the right is returned if FALSE. TRUE is the default value.
...
# Calculate the cumulative probability using the z-score, in our case we use:
...
::: {.callout-note style="border: 2px solid #f0ad4e;  border-radius: 8px;   background-color: #fff3cd;   padding: 10px;" appearance="simple" icon="false"}
...
It plays a similar role as the *standard deviation* for one observation from the population mean, remember that the Standard deviation measures the dispersion of individual data points around the mean of a dataset. It tells you how spread out the values in a dataset are.
...
If we are interested in the sum of n draws $S_n$, the sum and the average are related by $S_n = n\bar{x_n}$ so $SE(S_n)=\sqrt{n}\sigma$ which tells us that the variability of the sum of n draws increases at the rate of $\sqrt{n}$ so while increasing the sample size reduces the standard error of the average, it actually increases the standard error for the sum.
...
>
> The standard error of the sample sum is $SE(S_n)=\sqrt{n}\sigma$ , where n is the sample size and σ is the standard deviation of the population. Since n is 100 and we're told that the standard deviation of the population is \$37, we know the standard error of the sample sum is \$370.
>
...
for percentages, the expected value is $E = \mu * 100\%$ and the Standard Error $SE=\frac{\sigma}{\sqrt{n}} *100\%$
...
We are going to create 10,000 samples of N number of mice for N= 3,12,25 and 50 and calculate the difference for the means between treatment and control bodyweight and we calculate the t-statistic for each sample size (t-statistic is explained later)
...
We will see later that we don't usually calculate our p-values like this using the CLT, instead we will use a *t-test* that will give us a slightly different result. This is to be expected because our CLT approximation considered the denominator of tstat practically fixed (with large samples it practically is), while the t-distribution approximation takes into account that the denominator (the standard error of the difference) is a random variable. The smaller the sample size, the more the denominator varies.
...
where $\mu$ is the population mean, $\bar{x}$ is the observed sample mean and $E$ is a **margin of error.**
...
where SE is the *standard error* of the sample and if that is an average or a percentage then it is $SE= \frac{\sigma}{\sqrt{n}}$ .
...
> Example: We toss a coin 10 times, and get 7 tails. Using the formulas for the expected values of sums we have: Number of expected tails if the coin is fair:
>
...
::: {#SignificanceTestQuickFacts style="border: 2px solid #f0ad4e;  border-radius: 8px;   background-color: #fff3cd;   padding: 10px;"}
Quick Facts
...
to $CI= estimate (\mu) \mp t_{n-1}SE$
...
> Examples: in the attrition dataset. *Are the attrition proportions different for the two departments?*
>
> ```{r}
> kable(head(attrition1))%>%
>   kable_styling(latex_options = "scale_down")%>%
...
> ```
>
> In this case according to our p-value we cannot say that patients that had at least one psychiatric admision in the past have the same proportion of diagnosis than patients without any admission.
...
> refering to the substance_abuse data set:
>
...
The methods described above relate to **univariate** variables. In sciences it is common to be interested in the relationship between two or more variables. Relationships between variables are usually better visualized using scatterplots:
...
If the scatterplot shows a linear association, then this relationship between our data points can be summarized by a line. The equation for a line is $\hat{y}*i=a+bx_*i$. The idea is to choose the line that minimizes the sum of the squared distances between the observed $y_i$ and the $\hat{y_i}$

The method of least squares is the method we use to minimize the sum of a and b. This can be done like this\
$$
\sum_{i=1}^{n} (y_i - \hat{y}_i)^2 = \sum_{i=1}^{n} \left( y_i - (a + b x_i) \right)^2
$$ where $b$ is called slope and $a$ is the intercept. The intercept is the expected value of Y when X=0, in other words, the value at which the regression line crosses the Y axis it turns out that $b=r\frac{s_y}{s_x}$ where $r$ is the correlation coefficient and $s_y$ and $s_x$ are the two standard deviations and $a = \bar{y} - b\bar{x}$.

This line $\hat{y}=a+bx$ is called the regression line.
...
ggplot(data, aes(x = x, y = y)) +
  ...
  labs(title = "Scatter Plot with Regression Line and Reference Lines",
       x = "X-axis",
       y = "Y-axis")
...
When the value that deviates is the x value and not the y value we say that it is a high leverage point, and it has the potential to cause a big change in the regression line.
...
::: {#summarieswarn style="border: 2px solid #f0ad4e;  border-radius: 8px;   background-color: #fff3cd;   padding: 10px;"}
...
A correlation test checks the **null hypothesis that the population correlation** $\rho$ **is equal to 0**

The test also **assumes that the data isn't too skewed in any other direction and that there aren't extreme outliers**. As usual, a larger sample provides a certain degree of protection.
...
Regression analyses often report 'R-squared' It gives the fraction of the variation in the y-values that is explained by the regression line, so $1-r^2$ is the fraction of the variation in the y-values that is left in the residuals.
...
Dividing the error sum of square by its degrees of freedom we get what is called the **error mean square (MSE)** and measures the variability within the groups
